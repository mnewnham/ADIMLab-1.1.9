...
//主板版本和固件版本
//
  #if ENABLED(LCD_REV_MENU)
    void lcd_board_rev_menu();
    void lcd_FirmW_rev_menu();
    void lcd_rev_menu();
  #endif // LCD_REV_MENU
//**********************************************************
...
/**
 * Reset the status message 重置状态信息
 */
...
//    #endif
    ...
    if (planner.movesplanned() || IS_SD_PRINTING)
      MENU_ITEM(submenu, MSG_TUNE, lcd_tune_menu);            //“调节”
    else
      MENU_ITEM(submenu, MSG_PREPARE, lcd_prepare_menu);      //“准备”

    MENU_ITEM(submenu, MSG_CONTROL, lcd_control_menu);        //“控制”

    #if ENABLED(SDSUPPORT)                                    //“SD打印”
      if (card.cardOK) {                                  //如果SD卡检测OK
        if (card.isFileOpen()) {                            //如果SD卡文件已打开
          if (card.sdprinting)                                //如果SD卡文件正在打印
            MENU_ITEM(function, MSG_PAUSE_PRINT, lcd_sdcard_pause);     //菜单显示 “PAUSE PRINT”，暂停打印
          else                                                //如果SD卡文件没有打印          
            MENU_ITEM(function, MSG_RESUME_PRINT, lcd_sdcard_resume);   //菜单显示 “RESUME PRINT”,恢复打印
            #if ENABLED(ADVANCED_PAUSE_FEATURE)
              #if E_STEPPERS == 1 && !ENABLED(FILAMENT_LOAD_UNLOAD_GCODES)
...
//                  MENU_ITEM(submenu, MSG_FILAMENTCHANGE, lcd_temp_menu_e0_filament_change);
              ...
              #endif
            #endif
          MENU_ITEM(function, MSG_STOP_PRINT, lcd_sdcard_stop);  //菜单显示“STOP PRINT”，停止打印
        }
        else {                                    //如果SD卡没有被打开，则显示 “SD卡已插入”
    ...
    //
    // Set Case light on/off/brightness  //“照明LED”
    //
    #if ENABLED(MENU_ITEM_CASE_LIGHT)
      if (USEABLE_HARDWARE_PWM(CASE_LIGHT_PIN)) {
        MENU_ITEM(submenu, MSG_CASE_LIGHT, case_light_menu);
...
//*******************************************************
    #if ENABLED(LCD_REV_MENU)     //主板和固件版本号
      MENU_ITEM(submenu, MSG_REV_MENU, lcd_rev_menu);
    #endif
//*******************************************************
  ...
  /**
   * Watch temperature callbacks  温度监测反馈
   */
  #if HAS_TEMP_HOTEND             //喷嘴
    #if WATCH_HOTENDS
  ...
  // First Fan Speed title in "Tune" and "Control>Temperature" menus   //风扇速度
  ...
  // Refresh the E factor after changing flow    //挤出机 流量
  ...
  void lcd_tune_menu() {
    ...
    //
    // Manual bed leveling, Bed Z: “手动热床调平”---仅当开启MBL和LCD Bed Leveling时出现
    //
...
//    #endif
    ...
    // 此处 更换耗材 移动到 Info Screen界面下，与 Pause Print和Stop Print 同级别，方便操作
    // Change filament “更换耗材”
    //
//    #if ENABLED(ADVANCED_PAUSE_FEATURE)
//      #if E_STEPPERS == 1 && !ENABLED(FILAMENT_LOAD_UNLOAD_GCODES)
//        if (thermalManager.targetHotEnoughToExtrude(active_extruder))
...
//    #endif
  ...
  /**
   *
   * "Driver current control" submenu items “驱动电流控制” 子菜单---仅DAC_STEPPER_CURRENT
   *
   */
  #if ENABLED(DAC_STEPPER_CURRENT)
  ...
  void lcd_cooldown() {
  ...
  #if ENABLED(LEVEL_BED_CORNERS)    //如果使能 “调平热床边角”
    ...
    //*****************************************************************************
    //如果使能“MANUAL_BED_LEVEL”
 #if ENABLED(MANUAL_BED_LEVEL)   

    /**
     * Level points, starting in the front-left corner.
     */
    static int8_t bed_corner;
    void _lcd_goto_next_corner() {
      line_to_z(4.0);
   ...
   //////////////////////////////////////////////////////
    /**
     * Step 3: Start Level Bed Corners 
     */
    void _lcd_level_bed_corners() {
      defer_return_to_status = true;
      lcd_goto_screen(_lcd_corner_submenu);
   ...
   //////////////////////////////////////////////////////
     ...
     *         显示“Homing XYZ”,并等待回原点完成
     */
    void _lcd_level_bed_corners_homing() {
      if (lcdDrawUpdate) lcd_implementation_drawedit(PSTR(MSG_LEVEL_BED_HOMING), NULL);
      lcdDrawUpdate = LCDVIEW_CALL_NO_REDRAW;
      if (all_axes_homed()) lcd_goto_screen(_lcd_level_bed_corners);
    }
     ////////////////////////////////////////////////////

     ////////////////////////////////////////////////////
    /**
     * Step 1: Auto home XYZ...
     *         自动回原点
     */
    void _lcd_level_bed_corners_continue() {
      defer_return_to_status = true;
      axis_homed = 0;
...
//        else
         MENU_ITEM(submenu, MSG_MANUAL_BED_LEVEL, _lcd_level_bed_corners_continue);
      #endif
      //*************************************************
  ...
  void lcd_prepare_menu() {
    ...
    //菜单3：手动热床调平
    //******************************************************
    //如果使能“MANUAL_VED_LEVELING”&&不使能“LCD_BED_LEVELING”时，
    //在“prepare”菜单中增加“MANUAL_VED_LEVELING”选项
    #if ENABLED(MANUAL_BED_LEVEL) && DISABLED(LCD_BED_LEVELING)
//      if (all_axes_homed())   //如果XYZ回原点，则直接移动到第一点，否则，需要先回原点。
//        MENU_ITEM(function, MSG_LEVEL_CORNERS, _lcd_level_bed_corners); 
//      else 
        MENU_ITEM(function, MSG_MANUAL_BED_LEVEL, _lcd_level_bed_corners_continue);
    #endif  
    //******************************************************
    ...
    //X Height Adjust
    //
    #if ENABLED(X_HEIGHT_ADJUST)
//      if(all_axes_homed())
        MENU_ITEM(gcode, MSG_X_HEIGHT_ADJUST, PSTR("G28\nG1 Z28\nM84"));
    #endif
    //************************************************************************************* 

    //菜单3：TMC Z Calibration
    // TMC Z Calibration
    //
    ...
    #if ENABLED(Z_TOP)
//      if(all_axes_homed())
        MENU_ITEM(gcode, MSG_Z_TOP, PSTR("G28\nG1 Z390\nM84"));
    #endif
    //************************************************************************************* 
  ...
  /**
   *
   * "Control" > "Temperature" submenu   “控制” > “温度” 子菜单
   *
   */
  void lcd_control_temperature_menu() {
    START_MENU();
    ...
    /**
     *
     * "Temperature" > "Preheat Material 1 conf" submenu “温度” > “预热耗材1设置”
     *
     */
  ...
  /**
   *
   * "Control" > "Motion" submenu “控制” > “运动”子菜单 
   *
   */
  ...
  void lcd_control_motion_menu() {
    ...
    /**
     *
     * "Control" > "Filament" submenu  “控制” > “耗材” 子菜单
     *
     */
    void lcd_control_filament_menu() {
      START_MENU();
  ...
  /**
   *
   * "Control" > "Retract" submenu “控制” > “回抽” 子菜单
   *
   */
  #if ENABLED(FWRETRACT)
...
//***********************************************************************  
    ...
    /**
     *
     * VERSION(版本) > Board Rev.   
     *               > Firmware Rev.
     */
    void lcd_board_rev_menu() {
      if (use_click()) { return lcd_goto_previous_menu(); }
      START_SCREEN();
      STATIC_ITEM(BOARD_NAME, true, true);                           // Main Board
      STATIC_ITEM(BOARD_REV, true);                                 // REV
      STATIC_ITEM(MSG_INFO_BAUDRATE ": " STRINGIFY(BAUDRATE), true); // Baud: 250000
      STATIC_ITEM(R_SENSOR, true);                                  // A4988 Rsensor
      END_SCREEN();
    }
    ...
    void lcd_FirmW_rev_menu() {
      if (use_click()) { return lcd_goto_previous_menu(); }
      START_SCREEN();
      STATIC_ITEM(FIRMWARE_NAME, true, true);                           // Marlin Version
      STATIC_ITEM(FIRMWARE_REV, true );                                 // REV
      END_SCREEN();
    }
    /**
     *
    ...
    void lcd_rev_menu() {
      START_MENU();
      MENU_BACK(MSG_MAIN);
      MENU_ITEM(submenu, MSG_BOARD_REV_MENU, lcd_board_rev_menu);   // Board Rev > Name/Version/R-sensor
      MENU_ITEM(submenu, MSG_FIRMW_REV_MENU, lcd_FirmW_rev_menu);   // Firmware Rev >Malrlin119/Version
      END_MENU();
    }
  #endif // LCD_REV_MENU
//*********************************************************************
